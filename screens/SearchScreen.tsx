...
import {ObservableBaseFns, ObservableComputed} from '@legendapp/state';
...
function SearchScreen() {
  const search$ = useObservable({
    text: '',
    searchedCoins: [] as ObservableBaseFns<Coin>[],
  });
  ...
  function submitSearch() {
    ...
    const matchingCoins = coins$.filter((coin) =>
      coin.peek().name.toLowerCase().includes(searchInput.toLowerCase()),
    );
